...
@Component
public class BreakfastMenuPdfUtility {
    ...
    @Async
	public void breakfastMenuPdf(BreakfastMaster breakfastMaster, String currencySymbol, Boolean isItemized) throws Exception{
		String pdfFilePath = breakfastMenuPdfFinalLink(breakfastMaster.getMealSchool().getSchoolId(), 
				breakfastMaster.getYearMonth(), breakfastMaster.getRecId(), false);
		String logoPath = "";
		if(breakfastMaster.getMealSchool().getLogoLink() != null)
			logoPath = breakfastMaster.getMealSchool().getLogoLink();
		else
		...
		try{
			...
			document.open();
			document.add(createMealMenuPdf(pdfFilePath, logoPath, breakfastMaster,currencySymbol, isItemized));
			document.close();
	...
	private Element createMealMenuPdf(String pdfPath, String logoPath, BreakfastMaster breakfastMaster, String currencySymbol, Boolean isItemized)throws Exception{
		PdfPTable mainTable = new PdfPTable(1);
		PdfPCell cell=new PdfPCell();
		cell.addElement(createContentTable(pdfPath, logoPath, breakfastMaster,currencySymbol, isItemized));
		mainTable.addCell(cell);
		mainTable.setWidthPercentage(100);
		return mainTable;
	}
	
	private Element createContentTable(String pdfPath, String logoPath, BreakfastMaster breakfastMaster, String currencySymbol, Boolean isItemized) throws Exception{
		...
		first = new PdfPCell();
		first.setBorder(0);		
		first.addElement(createSchoolBox(breakfastMaster));
		mainTab.addCell(first);

		PdfPCell calenderView = new PdfPCell();
		calenderView.addElement(createCalendarTable(breakfastMaster,currencySymbol, isItemized));
	...
	private Element createBreakfastMenu(BreakfastMaster breakfastMaster, String currencySymbol, Boolean isItemized){
		calendarTable.setWidthPercentage(100);
		int yearVal = Integer.parseInt(breakfastMaster.getYearMonth().substring(0,4));
    	int monthVal = Integer.parseInt(breakfastMaster.getYearMonth().substring(4))-1;
    	...
    	while (position != calendar.get(Calendar.DAY_OF_WEEK)) {
    	    if(!CommonUtil.isWeekend(calendar,new ArrayList<>())){
    	...
    	while (day <= daysInMonth) {
    	    ...
    	    if(!CommonUtil.isWeekend(calendar,new ArrayList<>())){
    	    	List<BreakfastItems> breakfastItems = breakfastMaster.getBreakfastItems().stream()
    	    	        .filter(p ->  p.getBreakfastDate() != null && sdf1.format(/*dateUtility.convertGMTtoUserTime(
    	    	        		p.getStart())*/p.getBreakfastDate()).compareTo(sdf1.format(cal))==0)
    	    	        //.map(SchoolMeal::new)
	...
	private Element createCalendarTable(BreakfastMaster breakfastMaster, String currencySymbol, Boolean isItemized){
		calendarTable = new PdfPTable(5);
		PdfPCell calCell;
		List<String> daysArray = CommonUtil.getWeekDays(new ArrayList<>());
		/*if(!isItemized)
    	...
    	createBreakfastMenu(breakfastMaster,currencySymbol, isItemized);
		return calendarTable;
	}
	...
	public PdfPCell getDayCell(Calendar calendar, Locale locale, List<BreakfastItems> breakfastItems, String currencySymbol) {
	    PdfPCell cell = new PdfPCell();
	    ...
	    List<BreakfastItems> menuSides = new ArrayList<BreakfastItems>();
	    for(BreakfastItems breakfastItem : breakfastItems){
	    	if(breakfastItem.getItemType().toString().equalsIgnoreCase("BREAKFAST")){
	    		cellVal = new PdfPCell(new Phrase(breakfastItem.getItemName().toUpperCase(), generalFont));
	    		...
	    		cellVal.setBorder(0);
	 			cellTable.addCell(cellVal);
	    	}else if(breakfastItem.getItemType().toString().equalsIgnoreCase("HOLIDAY")){
	    		cellVal = new PdfPCell(new Phrase(breakfastItem.getItemName().toUpperCase(), generalFont));
				cellVal.setColspan(5);
				cellVal.setBorder(0);
	    		cellTable.addCell(cellVal);
	    		isHoliday = true;
	    	}else if(breakfastItem.getItemType().toString().equalsIgnoreCase("SIDE")){
	    		menuSides.add(breakfastItem);
	    	}
	    }	    
	    for(BreakfastItems mealMenu : menuSides){
	    	cellVal = new PdfPCell(new Phrase(mealMenu.getItemName().toUpperCase(), sideFont));
			cellVal.setColspan(5);
			cellVal.setBorder(0);
    		cellTable.addCell(cellVal);
	    }
	...
	private Element createSchoolBox(BreakfastMaster breakfastMaster){
		PdfPTable schTable = new PdfPTable(1);
		schTable.setWidthPercentage(150);
		PdfPCell schCell;
		schCell = new PdfPCell(new Phrase(breakfastMaster.getMealSchool().getSchoolName().toUpperCase(), boldFont));
		schCell.setBorder(0);
		schTable.addCell(schCell);
		schCell = new PdfPCell(new Phrase("BREAKFAST MENU FOR "+Month.of(
				Integer.parseInt(breakfastMaster.getYearMonth().substring(4))).name()+" "+
				breakfastMaster.getYearMonth().substring(0,4) , generalDateFont));
		schCell.setBorder(0);	
