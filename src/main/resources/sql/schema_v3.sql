DROP TRIGGER IF EXISTS mealOrdersAudit_calendarMenu_AFTER_DELETE ^; 
CREATE TRIGGER `mealOrdersAudit_calendarMenu_AFTER_DELETE` AFTER DELETE ON `mealOrdersAudit_calendarMenu` FOR EACH ROW BEGIN
SET SQL_SAFE_UPDATES=0;
DELETE FROM OrderMealItemsDetailReport WHERE
orderId=old.orderId AND schoolMealId=old.mealCalendarId;
